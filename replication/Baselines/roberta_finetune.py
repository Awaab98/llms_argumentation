...
# the following lines are for multiple GPUs communication, we only have one, so we dont need them
...
# this is for reproducibility, regarding the weights initialization and split
set_seed(42)
...
# the following are for model, and tokenizer downloadindg or using a chached saved version
...
# the .arrow file in a binary tabular format
# we need to convert the two colomns of argument1 and argument2 into one column of text
# and then do the tokenizaation
